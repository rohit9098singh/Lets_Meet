...
const ShowStoryPreview = ({
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        storyPreviewRef.current &&
        !storyPreviewRef.current.contains(event.target)
      ) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
      <div
        ref={storyPreviewRef}
        className="relative w-full max-w-md h-[70vh] flex flex-col bg-white dark:bg-gray-800 rounded-lg overflow-hidden"
      >
        <Button
          className="absolute top-4 right-4 z-10 text-gray-600 dark:text-gray-300 bg-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700"
          variant="ghost"
          onClick={onClose}
        >
          <X className="h-6 w-6" />
